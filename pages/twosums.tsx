...
const Home: NextPage = () => {
  ...
  function activeGame() {
    return picks.length < 6 && (picks.length > 0 && displayPicks.includes("_")) && timeLeft !== 0;
  }
  ...
  function picksUI() {
    return !winner() ? displayPicks.map((pick, i) => (<span className={!numbers.includes(parseInt(pick)) && !activeGame() ? 'text-red-600' : ""} key={i}>{ !numbers.includes(parseInt(pick)) && !activeGame() ? `${numbers[i]} ` : pick + " "}</span>)) : displayPicks.join(" ")
  }

  function startAgainBtn() {
    return <span onClick={() => resetGame()} className='cursor-pointer text-pink-500 font-semibold border-b-2 border-pink-500'>Start again</span>
  }
    ...
    <Layout>
      ...
      <div className='text-center pt-8 max-w-xl mx-auto'>
        ...
        <div className='mb-8 grid grid-cols-5 gap-2 px-10 md:px-20'>
          {
            keypad.map(key => (
              <button key={key.number} onClick={() => pickNumber(key.number)} 
                className={`${key.status === 'wrong' ? "bg-gray-700" : key.status === "right" ? "bg-green-500" : "bg-pink-700 hover:bg-pink-500"} md:p-3 p-2 py-4 rounded-md lg:text-xl sm:text-sm md:text-md font-bold transition-all md:min-w-[2.5rem] min-w-[1.85rem] text-white mt-1 mr-1`} 
                disabled={timeLeft === 0 || key.status !== "unknown" || !(picks.length >= 0 && picks.length < 6) || (picks.length > 0 && !displayPicks.includes("_"))} 
              >{key.number}</button>
            ))
          }
        </div>
        ...
        {
          picks.length === 0 ? null 
            : activeGame() ? <p>{6 - picks.length} {picks.length === 4 ? "pick" : "picks"}  remaining. {timeLeft} seconds left.</p>
            : winner() ? <div>You won! {startAgainBtn()}</div>
